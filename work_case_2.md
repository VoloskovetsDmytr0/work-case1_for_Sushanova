...
## 🏗️ 1. Встановіть на своїй домашній робочій станції гіпервізор ІІ типу – Virtual Box, VMWare Workstation, Hyper-V (або інший на Ваш вибір).

Для виконання роботи було обрано **Oracle VM VirtualBox**. Це популярний гіпервізор ІІ типу, що дозволяє створювати ізольовані середовища для тестування ПЗ. (а ще я користуюсь ним з 2017 року :3)
...
## ⚙️ 2. Створення нової віртуальної машини
...
## 🛠️ 4. Встановіть у мінімальній конфігурації з термінальним вводом-виводом без графічного інтерфейсу операційну систему GNU/Linux
...
### 4.3 Встановіть додатково ще другу графічну оболонку та порівняйте її можливості з GNOME.
...
## Порівняння робочих середовищ: GNOME проти XFCE

## 📊 Порівняльна таблиця

| Критерій | GNOME | XFCE |
| :--- | :--- | :--- |
| **Версія GTK** | GTK4 / Libadwaita | GTK3 |
| **Споживання RAM (простій)** | ~800 МБ — 1.2 ГБ | ~400 МБ — 600 МБ |
| **Філософія** | Робочий процес без відволікань | Традиційний десктоп |
| **Складність налаштування** | Потребує розширень (Extensions) | Глибока вбудована кастомізація |
| **Підтримка Wayland** | Відмінна (найкраща в Linux) | Часткова (у процесі розробки) |
| **Жести тачпада** | Підтримуються "з коробки" | Потребують стороннього софту |

---

## 🏗️ Ключові особливості

### 1. GNOME: Сучасність та мінімалізм
GNOME пропонує унікальний досвід користувача, який відрізняється від Windows чи macOS.

* **Activities Overview**: Центр керування вікнами та програмами.
* **Дизайн**: Програми виглядають цілісно завдяки бібліотеці `Libadwaita`.
* **Інтеграція**: Найкраща підтримка онлайн-акаунтів (Google, Nextcloud) та системних сповіщень.

### 2. XFCE: Стабільність та швидкість
XFCE називають "мишею", бо воно швидке і не заважає працювати.

* **Модульність**: Кожен елемент (панель, менеджер вікон) є окремим процесом.
* **Thunar**: Швидкий та функціональний файловий менеджер.
* **
